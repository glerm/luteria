...
%\local{Brasil}
\data{12 de fevereiro de 2015, v0.9-Defesa}
...
..... inserir aqui resumo e comentários sobre transformações do ciclo de terças menores e terças maiores exemplificados nos patches e explicados em \citeonline[ p.23-25]{susanni_antokoletz2012music}
...
Em seu artigo \textit{"The unfolding of tonality in the music of Bela Bartók"},\ David \citeonline{cooper1998unfolding} propõe a análise assistida por computador das \textit{"10 Easy Piano Pieces"} de Bartók. Após testar os mesmos algoritmos de detecção de tonalidade que usou para analisar as peças do \textit{"Cravo Bem Temperado"} de J. S. Bach, onde obteve uma aproximação muito satisfatória das análises conhecidas da obra, Cooper conclui que mesmo com alguns desvios previstos o procedimento ajuda o analista a inferir ideias sobre ambiguidade ou politonalidade em compositores pós-tonais como Bartók. Por ser independente de "estilo" o algoritmo poderia ajudar a revelar estruturas implícitas.

\begin{citacao}
Although in itself the algorithm is fundamentally musically ignorant, 'knowing' little about musical structure and nothing of aesthetics, it is able to simulate quite successfully one particular musical
skill, that of determining local and global tonality. Indeed, it is because of its artlessness that it could form a useful tool in the analyst's repertoire, by providing a measure of tonality which is largely independent of 'style dependent' knowledge.\cite[ p.34-35]{cooper1998unfolding}
\end{citacao}
...
O módulo de análise\footnote{c.f. Código em \url{https://github.com/cuthbertLab/music21/blob/master/music21/analysis/discrete.py}. Acesso em 13 de Fevereiro de 2015.} de tonalidade do Music21 utiliza como padrão algoritmo de "Krumhansl-Schmuckler"\footnote{c.f. Discussão na mailing list do Music21 sobre o algoritmo em \url{https://groups.google.com/forum/\#!topic/music21list/lwamYR0o8To}. Acesso em 13 de Fevereiro de 2015.},     
...
% sobre a sistribuição das classes de nota
\lipsum[5]


\subsubsection{Escalas e Modalismo}

%sobre  a importancia do pensamento modal, pentatonica e suas rotações
\lipsum[6]

\begin{figure}[!h]
	\caption{\label{fig_grafico}A sugestão de exercício das "escalas"\ de Mikrokosmos 109 na partitura original revela a saliência da sequencia de 2-2-2-1 intervalos de semitom a partir de Sol que remetem ao modo lídio. No entanto o Sol lídio teria que ter Fá sustenido: fica assim explícita a invenção de um modo derivado ou "polimodo". } 
	\begin{center}
...
% sobre as vantagens de poder analisar os intervalos algoritmicamente - o humanamente possivel 

\lipsum[7]

%sobre  a importancia do pensamento modal, pentatonica e suas rotações

No ínicio da melodia de Mikrokosmos 109 uma relação "quase"\ pentatônica polariza a emergência da nota Sol por sua ausência no intervalo pentatônico de 3-2-3 semitons que faria os passos Si-Ré-Mi-(Sol). A inserção dos semitons Fa e Dó\# funciona como um "resumo" das relações semitonais da polimodalismo da composição. A nota Sol nunca será usada na melodia, mas durante quase toda composição figura no baixo.

\begin{figure}[!h]
	\caption{\label{fig_grafico} Análise de contorno intervalar na melodia inicial de Mikrokosmos 109.  } 
	\begin{center}
...
%sobre a evidenência da estratégia lídio - mixolídio no final

\lipsum[8]

\begin{figure}[!h]
	\caption{\label{fig_grafico}O final de Mikrokosmos 41 evidencia a sequencia de 2-2-2-1 semitons do modo Lídio. } 
	\begin{center}
...
O final de Mikrokosmos 41 cita o clichê da sequencia de 2-2-2-1 semitons do modo Lídio e "resolve" o acorde de Sol maior sem o uso da sensível Fá\#, negando-se a uma evidência de tonalidade maior tradicional. A estratégia é pela condução da quinta justa Ré através dos passos Dó\# $\rightarrow $ Ré e Dó\# $\rightarrow $ Si mantendo-se em díade até que uma última entrada da nota Sol no baixo deixe uma impressão vaga de que havia uma intenção modal em torno do acorde de Sol maior 
