...
\begin{figure}[!h]
	\caption{\label{fig_grafico}Módulo de segmentação para análise dentro dos objetos chord-seq. }
	\begin{center}
	    \includegraphics[scale=0.44]{OM_settheory/SegmentaMikro109Octa.png}
	\end{center}
	\legend{Fonte: autor }
\end{figure}	
...
No código\footnote{A especificação de classes e métodos de analise disponíveis esta em \url{http://repmus.ircam.fr/openmusic/dev-resources/analysis}. Acesso em 3 de março de 2015.} temos: 

\begin{itemize}

\item A definição do escopo com \textit{in-package:om} para a carga das classes e métodos

\item A definição de classe uma análise \textit{(defclass! nome-da-analise (abstract-analysis)()))}. 
