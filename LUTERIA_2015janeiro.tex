...
Isso logo inspira também as expansões de técnicas através de aplicações de teorias matemáticas mais avançadas sobre a serialização de eventos partituráveis. No entanto este serialismo integral e ortodoxo vai aos poucos sendo permeado por uma música computacional que estimula a busca por sonoridades cada vez menos restritas a organização das 12 alturas da escala temperada. 

O computador passa a ser cada vez mais usado para a síntese sonora e em dado momento sua capacidade de memória passa a permitir também as manipulações muito avançadas em sons gravados. Os experimentos concretos e eletrônicos com sonoridades passam a ser uma das primeiras coisas que um músico iniciado na computação terá contato, já que o uso dos processos partiturados estarão sempre de alguma forma ligados a tradição da orquestração ou estudo de instrumentos musicais de um repertorio

Jean Bresson, um dos desenvolvedores da linguagem OpenMusic, mantida pelo IRCAM, aponta que esta tendência para uso do computador como ferramenta de manipulação de sonoridades sintéticas ou pré-gravadas tem seu auge justamente neste período que sucede a possibilidade do uso do computador pessoal, na contramão do que passa a ser considerada a sonoridade da "computer music", focando em processos simbólicos de manipulação:
...
parece cada vez mais seduzir para um estudo da música que inicia pela "morfologia do espectro sonoro"\cite{smalley1997spectromorphology} , onde impera o "microsom"\cite{roads2004microsound}\ manipulado nos níveis limites da psicoacústica.




Interessa em nossa abordagem aqui repensar um pouco os meandros desta manipulação simbólica de escalas e aglomerados de alturas, antes de mergulhar nas possibilidades expandidas pelo computador. 

O objeto central de estudo desta pesquisa são os algoritmos de manipulação de dados de uma superfície musical normatizada para a escala de temperamento igual, de doze tons \footnote{Conhecida também como "12-TET" ou "EDO"\cite[ p. 76]{sethares2005tuning}}, tradicionalmente usada nas partituras da prática comum do repertório ocidental. Estamos abstraindo no momento as questões como timbre, microtonalismo,  ou quaisquer outros aspectos não formalizáveis em partitura tradicional. 
